...
def get_interest_rate_backwards(deals, target_name, date_col):
    monthly = deals.set_index(date_col)[target_name].resample("M").mean().to_period("M")
    pct = monthly.pct_change().fillna(0)
    cum_rev = (1 + pct).iloc[::-1].cumprod().iloc[::-1]
    return cum_rev
...
def main():
    ...
    if "Сумма договора" in df.columns and "Дата регистрации" in df.columns:
        ir_full = get_interest_rate(df, "Сумма договора", "Дата регистрации")
        df["Сумма договора (disc)"] = discounting(df, ir_full, "Сумма договора", "Дата регистрации")

    targets = [c for c in ("Сумма договора", "Сумма договора (disc)", "Площадь договора") if c in df.columns]
    ...
    if section == "Обзор признаков":
        ...
        for col in all_feats + targets:
            ...
            if remove_outliers:
                if is_numeric_dtype(data_col):
                    ...
                    low_dt, high_dt = pd.to_datetime(low_i), pd.to_datetime(high_i)
                    data_col = data_col[data_col.between(low_dt, high_dt)]

            c1, c2 = st.columns(2)
            if col == "Телефон":
                vals = data_col.astype(str).str.replace(r"\D", "", regex=True).str[:3]
                ...
                with c1:
                    try:
                        stats = dates.describe(datetime_is_numeric=True)
                    except TypeError:
                        stats = dates.describe()
                    st.write(stats)
                with c2:
                    fig = px.histogram(x=months, nbins=bins, histnorm=histnorm)
                    fig.update_layout(height=200, margin=dict(l=0, r=0, t=0, b=0), xaxis_title="Месяц")
                    st.plotly_chart(fig, use_container_width=True)
        ...
        if "Типология" in data.columns:
            data = data[data["Типология"] != "СП"]
        ...
        if remove_feature:
            if is_datetime64_any_dtype(data[feature]):
                ...
                low_i, high_i = np.quantile(dt_int, [0.01, 0.99])
                low_dt, high_dt = pd.to_datetime(low_i), pd.to_datetime(high_i)
                data = data[data[feature].between(low_dt, high_dt)]
        ...
        if target == "Сумма договора" and "Дата регистрации" in data.columns:
            ir_back = get_interest_rate_backwards(data, target, "Дата регистрации")
            data["Сумма договора (disc)"] = discounting(data, ir_back, target, "Дата регистрации")
        ...
        if is_datetime64_any_dtype(data[feature]) or feature == "Возраст":
            ...
            st.plotly_chart(
                px.scatter(df_raw, x="x", y=target).update_layout(xaxis_title=feature, yaxis_title=target),
            ...
            fig.update_layout(xaxis=dict(range=[0,1]), yaxis=dict(range=[0,1]), height=400, margin=dict(l=0,r=0,t=30,b=0))
            st.plotly_chart(fig, use_container_width=True)
            ...
            with col1:
                ...
                st.plotly_chart(
                    px.scatter(avg_df, x="x", y=target).update_layout(xaxis_title=feature, yaxis_title=target),
                    use_container_width=True
                )
            with col2:
                st.subheader("Среднее по году")
                st.plotly_chart(
                    px.line(avg_df, x="x", y=target).update_layout(xaxis_title=feature, yaxis_title=target),
                    use_container_width=True
                )

            st.subheader(f"Распределение {feature}")
            st.plotly_chart(
                px.histogram(x=x_val, nbins=bins).update_layout(xaxis_title=feature, yaxis_title="count"),
                use_container_width=True
            )
            ...
            st.plotly_chart(fig, use_container_width=True)
            st.plotly_chart(
                px.box(data.assign(grp=grp), x="grp", y=target, category_orders={"grp": cats}),
                use_container_width=True
            )
